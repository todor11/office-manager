...
function createNewEmployee() {
    ...
    for (var i = 0; i < phoneContainerChilds.length; i++) {
        ...
        var phoneNumberId = $(phoneContainer).attr('phoneNumberId');
        var isNewPhoneNumber = phoneNumberId == undefined;
        if (isNewPhoneNumber) {
            ...
            //var blockIndexElements = $(phoneContainer).attr('id').split('_');
            //var blockIndex = blockIndexElements[blockIndexElements.length - 1];
            var phoneNumber = $('#empl-phone-number-input_' + phoneNumberId).val();
            if (RegExPatternConstants.PHONE_NUMBER.test(phoneNumber)) {
                var phoneType = $('#empl-phone-type_' + phoneNumberId).val();
                if (phoneType == '') {
                    EPPZScrollTo.scrollVerticalToElementById('empl-phone-type_' + phoneNumberId);
                    stopAnimationAndEnableButton(buttonEl);
                    var msg = 'Типа на телефонния номер е задължителен !';
                    negativeAlertBox(null, msg, null, null);
                    return;
                ...
                var description = $('#phone-description_' + phoneNumberId).val();
                var phone = {};
    ...
    employee.id = $('#empl-main-button').attr('employeeId');
    employee.isActive = checkBoolean($('#isActive'));
